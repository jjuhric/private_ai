...
export default function App({ toolLogs: propToolLogs, activeAgent: propActiveAgent, isStreaming: propIsStreaming }) {
  ...
  // Coding & AI Academy state
  const [academyLessons, setAcademyLessons] = useState([]);
  ...
  const fetchAcademyLessons = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setAcademyLessons(data);
      } else {
        const errData = await res.json();
        setAcademyError(errData.error || 'Failed to fetch academy lessons');
      }
    } catch (err) {
      setAcademyError('Network error fetching academy lessons');
    } finally {
  ...
  const fetchLessonDetails = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errData = await res.json();
        setAcademyError(errData.error || 'Failed to fetch lesson details');
      }
    } catch (err) {
      setAcademyError('Network error fetching lesson details');
    } finally {
      setLoadingAcademy(false);
    }
  };

  const startAcademyLesson = async (e) => {
    e.preventDefault();
    ...
    try {
      const res = await fetch('/api/academy/start', {
        ...
        body: JSON.stringify({ language: selectedLang, topic: courseTopic })
      });
      if (res.ok) {
        const data = await res.json();
        setCourseTopic('');
        await fetchLessonDetails(data.lessonId);
        fetchAcademyLessons();
      } else {
        const errData = await res.json();
        setAcademyError(errData.error || 'Failed to generate curriculum');
      }
    } catch (err) {
      setAcademyError('Network error starting academy lesson');
    } finally {
      setLoadingAcademy(false);
    }
  };

  const submitAcademyAnswer = async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      if (res.ok) {
        ...
        const errData = await res.json();
        setAcademyError(errData.error || 'Failed to submit solution');
      }
    } catch (err) {
      setAcademyError('Network error submitting solution');
    } finally {
      setSubmittingAnswer(false);
    }
  };

  const pauseAcademyLesson = async (id) => {
    try {
  ...
  const resumeAcademyLesson = async (id) => {
    try {
  ...
  return (
    <div className="memory-pane" style={{
      ...
      <div className="section-header" style={{
        ...
        <div className="scroll-tab-container" style={{
          ...
          >
            ...
            <button 
              className={`btn btn-secondary ${activeSubTab === 'academy' ? 'active' : ''}`}
      ...
      {activeSubTab === 'academy' && (
        <div style={{ display: 'flex', gap: '20px', minHeight: '600px', flexWrap: 'wrap' }}>
          {/* Academy Left History Sidebar */}
          <div className="memory-card" style={{ flex: '1 1 280px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px', maxHeight: '75vh', overflowY: 'auto' }}>
            <h3 style={{ fontSize: '1rem', color: '#fff', margin: 0, borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: '10px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <Trophy size={16} style={{ color: 'var(--accent-primary)' }} />
              Course Progress & History
            </h3>
            
            {loadingAcademy && academyLessons.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {academyLessons.map((l) => {
                  ...
                  return (
                    ...
                    >
                      <div style={{ fontWeight: 600, color: '#fff', fontSize: '0.88rem', textTransform: 'capitalize' }}>
                        {l.language} Academy
                      </div>
                      <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '2px', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>
                        {l.topic}
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '8px', fontSize: '0.75rem' }}>
                        <span style={{ 
                          color: l.status === 'completed' ? '#34d399' : (l.status === 'paused' ? '#fbbf24' : 'var(--accent-primary)'),
                          fontWeight: 600
                        }}>
                          {l.status.toUpperCase()}
                        </span>
                        {l.status === 'completed' && l.overall_grade && (
                          <span style={{ color: '#fff', background: 'rgba(52,211,153,0.15)', padding: '2px 6px', borderRadius: '4px' }}>
              ...
              <div style={{ color: 'var(--text-secondary)', textAlign: 'center', padding: '20px 0', fontSize: '0.85rem' }}>
                No lessons started yet. Start one on the right!
              </div>
            )}
          ...
          {/* Academy Main Panel */}
          <div className="memory-card" style={{ flex: '3 3 500px', padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {academyError && (
              <div style={{ padding: '12px', background: 'rgba(239,68,68,0.15)', border: '1px solid #ef4444', color: '#fca5a5', borderRadius: '6px', fontSize: '0.85rem' }}>
                {academyError}
              </div>
            )}
            ...
            {!activeLesson ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div>
                  <h2 style={{ fontSize: '1.25rem', color: '#fff', margin: '0 0 6px 0' }}>Launch a Planned Teaching Route 🚀</h2>
                ...
                <form onSubmit={startAcademyLesson} style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginTop: '10px' }}>
                  <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
                    ...
                    <div className="form-group" style={{ flex: '2 2 300px', margin: 0 }}>
                      ...
                      <input 
                        type="text" 
                        className="form-control" 
                        placeholder="e.g. Create a simple rust application" 
                        required 
                        value={courseTopic} 
                  ...
                  >
                    {loadingAcademy ? 'Designing Curriculum...' : 'Start Learning Route'}
                  </button>
                </form>
              </div>
            ) : (
              /* Active / Selected Lesson View */
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '10px', borderBottom: '1px solid rgba(255,255,255,0.08)', paddingBottom: '16px' }}>
                  <div>
                    <span style={{ fontSize: '0.8rem', background: 'var(--accent-primary)', color: '#fff', padding: '2px 8px', borderRadius: '4px', textTransform: 'uppercase', fontWeight: 600 }}>
                      {activeLesson.language}
                    </span>
                    <h2 style={{ fontSize: '1.25rem', color: '#fff', margin: '6px 0 2px 0' }}>{activeLesson.topic}</h2>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    {activeLesson.status === 'active' ? (
                      <button 
                        className="btn btn-secondary" 
                        onClick={() => pauseAcademyLesson(activeLesson.id)}
                      ...
                      <button 
                        className="btn btn-primary" 
                        onClick={() => resumeAcademyLesson(activeLesson.id)}
                        style={{ padding: '6px 12px', fontSize: '0.8rem' }}
                      >
                        ▶️ Resume Course
                      </button>
                ...
                {activeLesson.status === 'completed' ? (
                  ...
                  <div style={{ textAlign: 'center', padding: '30px 10px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
                    ...
                    <h3 style={{ color: '#fff', margin: 0 }}>Course Completed!</h3>
                    ...
                    <div style={{ display: 'flex', gap: '20px', marginTop: '10px', background: 'rgba(255,255,255,0.05)', padding: '16px 30px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.08)' }}>
                      <div>
                        <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Overall Grade</div>
                        <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#34d399' }}>{activeLesson.overall_grade}%</div>
                      </div>
                      <div style={{ borderLeft: '1px solid rgba(255,255,255,0.1)' }}></div>
                      <div>
                        <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Certification Rating</div>
                        <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#fff' }}>{activeLesson.overall_rating}</div>
                      </div>
                    </div>
                  </div>
                ) : (
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          <span style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                            Progress: Step {stepIdx + 1} of {activeLesson.curriculum.length}
                          </span>
                        ...
                        <div style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '20px' }}>
                          <h3 style={{ fontSize: '1.1rem', color: '#fff', margin: '0 0 10px 0' }}>{step.title}</h3>
                          
                          {/* Rich explanation - pre-wrap to support long detailed paragraphs and spacing */}
                          <div style={{ color: 'rgba(255,255,255,0.85)', fontSize: '0.92rem', lineHeight: '1.6', whiteSpace: 'pre-wrap', marginBottom: '16px' }}>
                            {step.explanation}
                          </div>

                          {step.code_example && (
                            <div style={{ marginTop: '14px' }}>
                              <h4 style={{ fontSize: '0.85rem', color: 'var(--accent-primary)', margin: '0 0 6px 0' }}>Illustrative Code Example</h4>
                              <pre style={{ margin: 0, padding: '12px', background: 'rgba(0,0,0,0.25)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '8px', fontSize: '0.82rem', color: '#ddd', overflowX: 'auto', fontFamily: 'var(--font-mono)' }}>
                                {step.code_example}
                              </pre>
                            </div>
                          )}
                          ...
                          {step.exercise && (
                            <div style={{ marginTop: '16px', background: 'rgba(255,255,255,0.02)', padding: '12px', borderRadius: '6px', borderLeft: '3px solid var(--accent-primary)' }}>
                              <h4 style={{ fontSize: '0.85rem', color: '#fff', margin: '0 0 4px 0' }}>Lesson Exercise</h4>
                              <p style={{ margin: 0, fontSize: '0.88rem', color: 'var(--text-secondary)' }}>{step.exercise}</p>
                            </div>
                          )}
                        </div>

                        {/* Final Test Challenge Section */}
                        <div>
                          <h3 style={{ fontSize: '1rem', color: '#fff', margin: '0 0 8px 0', display: 'flex', alignItems: 'center', gap: '6px' }}>
                          ...
                          {grade ? (
                            ...
                            <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', padding: '16px', marginBottom: '16px' }}>
                              <div style={{ display: 'flex', justifycontent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                                <span style={{ fontWeight: 600, fontSize: '0.9rem', color: '#fff' }}>Grading Result</span>
                                <span style={{ 
                                  fontSize: '1rem', 
                                  fontWeight: 'bold', 
                                  color: grade.score >= 70 ? '#34d399' : '#ff6b6b',
                                  background: grade.score >= 70 ? 'rgba(52,211,153,0.12)' : 'rgba(255,107,107,0.12)',
                                  padding: '4px 10px',
                                  borderRadius: '6px'
                                }}>
                              ...
                              <p style={{ margin: 0, fontSize: '0.88rem', color: '#ddd', lineheight: '1.5', whiteSpace: 'pre-wrap' }}>
                                {grade.feedback}
                              </p>
                            ...
                            /* Submit Answer Form */
                            <form onSubmit={submitAcademyAnswer} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                              <div className="form-group" style={{ margin: 0 }}>
                                <label>Write your solution code here</label>
                                <textarea
                                  rows={8}
                                  className="form-control"
